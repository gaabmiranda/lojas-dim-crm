...
__export(schema_exports, {
  ...
  blingTokens: () => blingTokens,
  cards: () => cards,
  colunaCardEnum: () => colunaCardEnum,
  ...
  pedidos: () => pedidos,
  roleUsuarioEnum: () => roleUsuarioEnum,
  statusAtividadeEnum: () => statusAtividadeEnum,
  templatesMensagem: () => templatesMensagem,
  tipoCardEnum: () => tipoCardEnum,
  usuarios: () => usuarios
});
var tipoCardEnum = pgEnum("tipo_card", ["pos_venda", "reativacao"]);
var colunaCardEnum = pgEnum("coluna_card", [
  "pendente",
  "em_contato",
  "finalizado",
  "arquivo"
]);
var statusAtividadeEnum = pgEnum("status_atividade", [
  "pendente",
  "em_andamento",
  "concluida",
  "cancelada"
]);
var roleUsuarioEnum = pgEnum("role_usuario", ["admin", "vendedor"]);
var contatos = pgTable(
  "contatos",
  {
...
var pedidoItens = pgTable("pedido_itens", {
  id: serial("id").primaryKey(),
...
var cards = pgTable(
  ...
  (t) => ({
    ...
    contatoAtivoUnique: uniqueIndex("cards_contato_ativo_unique").on(t.contatoId).where(sql`${t.coluna} != 'arquivo'`)
  })
);
var atividades = pgTable(
  "atividades",
  {
    id: serial("id").primaryKey(),
...
var eventos = pgTable(
  ...
  (t) => ({
    origemExternalUnique: uniqueIndex("eventos_origem_external_unique").on(t.origem, t.externalId).where(sql`${t.externalId} IS NOT NULL`),
    cardIdx: index("eventos_card_idx").on(t.cardId),
    criadoIdx: index("eventos_criado_idx").on(t.criadoEm.desc())
  })
);
var comentarios = pgTable("comentarios", {
  id: serial("id").primaryKey(),
  cardId: integer("card_id").notNull().references(() => cards.id, { onDelete: "cascade" }),
...
var templatesMensagem = pgTable("templates_mensagem", {
  key: text("key").primaryKey(),
  descricao: text("descricao"),
  conteudo: text("conteudo").notNull(),
  atualizadoEm: timestamp("atualizado_em", { withTimezone: true, mode: "date" }).notNull().defaultNow()
});
var notificacoes = pgTable(
  "notificacoes",
  {
    id: serial("id").primaryKey(),
...
__export(relations_exports, {
  atividadesRelations: () => atividadesRelations,
  cardsRelations: () => cardsRelations,
  comentariosRelations: () => comentariosRelations,
  contatosRelations: () => contatosRelations,
  eventosRelations: () => eventosRelations,
  notificacoesRelations: () => notificacoesRelations,
  pedidoItensRelations: () => pedidoItensRelations,
  pedidosRelations: () => pedidosRelations,
  usuariosRelations: () => usuariosRelations
});
var contatosRelations = relations(contatos, ({ many }) => ({
  pedidos: many(pedidos),
  cards: many(cards),
  eventos: many(eventos)
}));
var pedidosRelations = relations(pedidos, ({ one, many }) => ({
...
var pedidoItensRelations = relations(pedidoItens, ({ one }) => ({
  pedido: one(pedidos, {
...
var atividadesRelations = relations(atividades, ({ one }) => ({
  card: one(cards, {
...
var eventosRelations = relations(eventos, ({ one }) => ({
  card: one(cards, {
...
var comentariosRelations = relations(comentarios, ({ one }) => ({
  card: one(cards, {
...
var notificacoesRelations = relations(notificacoes, ({ one }) => ({
  usuario: one(usuarios, {
...
function buildPool() {
  ...
  if (!url) {
    throw new Error("DATABASE_URL n\xE3o configurado no ambiente.");
  }
...
var TEMPLATES = [
  {
    key: "pos_venda_d14",
    descricao: "Mensagem D+14 ap\xF3s entrega do pedido.",
    conteudo: [
      "Oi {{nome_cliente}}, tudo bem? \u{1F60A}",
      "",
      "Aqui \xE9 da Lojas Dim. Faz uns 14 dias que voc\xEA recebeu seu pedido com {{primeiro_item}}.",
      "",
      "Est\xE1 tudo certo? Qualquer coisa que precise, \xE9 s\xF3 me chamar por aqui."
    ].join("\n")
  },
  {
    key: "reativacao_1",
    descricao: "Primeira tentativa de reativa\xE7\xE3o (D+90 ap\xF3s p\xF3s-venda).",
    conteudo: [
      "Ol\xE1 {{nome_cliente}}! Aqui \xE9 da Lojas Dim. \u{1F44B}",
      "",
      "Faz um tempinho que n\xE3o te vejo por aqui. Chegou retalho novo essa semana que combina com o que voc\xEA levou da \xFAltima vez.",
      "",
      "Quer dar uma olhada?"
    ].join("\n")
  },
  {
    ...
    conteudo: [
      "Oi {{nome_cliente}}, sumido(a)! \u{1F642}",
      "",
      "Estamos com novidades na loja. Posso te mandar algumas op\xE7\xF5es pra dar uma olhada sem compromisso?"
    ].join("\n")
  },
  {
    key: "reativacao_3",
    descricao: "Terceira e \xFAltima tentativa antes de arquivar.",
    conteudo: [
      "Oi {{nome_cliente}}, \xE9 da Lojas Dim. \u2728",
      "",
      "\xDAltima chamada pra esse m\xEAs \u2014 se quiser, te mando uma sele\xE7\xE3o das melhores pe\xE7as que chegaram. Caso prefira, me avisa que pauso os contatos por aqui."
    ].join("\n")
  }
];
async function main() {
  for (const t of TEMPLATES) {
    await db.insert(templatesMensagem).values(t).onConflictDoUpdate({
